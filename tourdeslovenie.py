...
def razdalja(t1, t2):
    ...
    dolzina_razdalje = sqrt((kateta1 - kateta3)**2 + (kateta4 - kateta2)**2)
    return dolzina_razdalje
#3

def oddaljenost(kraj1, kraj2, koordinate):
    koordinata_k1 = podatek(kraj1, koordinate)
    koordinata_k2 = podatek(kraj2, koordinate)

    oddaljenost_krajev = razdalja(koordinata_k1, koordinata_k2)
    return oddaljenost_krajev

#4

def visina(kraj1, kraj2, visine):
    vis1 = podatek(kraj1, visine)
    vis2 = podatek(kraj2, visine)
    dejanska_visinka = vis2 - vis1
    return dejanska_visinka
#5
def cas(s, h):
    cas_tripa = s * 2.4
    ...
    if h > 0:
        ...
        cas_spusta = 1/250 * h
        cas_tripa += cas_spusta
    return cas_tripa
#6

def cas_med(kraj1, kraj2, koordinate, visine):
    x1 = podatek(kraj1, koordinate)
    x2 = podatek(kraj2, koordinate)
    y = oddaljenost(kraj1, kraj2, koordinate)
    visinska = visina(kraj1, kraj2, visine)
    dejanski_cas =cas(y, visinska)
    return dejanski_cas
...
def  skupni_cas(kraji, koordinate, visine):
    ...
    for kraj in kraji:
        kraj_real = podatek(kraj, koordinate)
        s_koordinat.append(kraj_real)
        s_krajev.append(kraj)
    for oddaljenosti in range(len(s_krajev)):
        if oddaljenosti != len(s_krajev) - 1:
            kraj1 = s_krajev[oddaljenosti]
            kraj2 = s_krajev[oddaljenosti + 1]
            cel_čas = cas_med(kraj1, kraj2, koordinate, visine)
            sestevek_casa += cel_čas
    return sestevek_casa
