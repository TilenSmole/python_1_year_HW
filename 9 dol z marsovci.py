...
def najblizja(marsovec: Koordinate, ladje: Flota) -> Koordinate:
    ...
    """
    distance = razdalja(marsovec, ladje[0])
    for ladja in ladje:
        distanca_ladja_marsovc = (razdalja(marsovec, ladja))

        if distanca_ladja_marsovc < distance and ladja == ladje[0]:
            distance = distanca_ladja_marsovc
            najblizji = ladja


        elif distanca_ladja_marsovc <= distance:
            distance = distanca_ladja_marsovc
...
def dodeli_ladje(marsovci: Flota, ladje: Flota) -> Flota:
    ...
    for marsovc in marsovci:
        naj = najblizja(marsovc, ladje)
        s_dodeljenih_ladij.append(naj)

    return (s_dodeljenih_ladij)
...
def skupine_marsovcev(marsovci: Flota, ladje: Flota) -> Skupine:
    ...
    import collections
    kdo_koga = {}
    #poglej vse ladje pa glej kera bo najblizja
    pogostosti = collections.defaultdict(set)
    for marsovc in marsovci:
        t = najblizja(marsovc, ladje)
        for ladja  in ladje:
            if t == ladja:
                pogostosti[t].add(marsovc)


    return (pogostosti.values())
...
def razpostavi_ladje(skupine: Skupine) -> Flota:
    ...
    optimalna_razporeditev = []
    for ladje in skupine:
        ladje = list(ladje)
        sredisce_ladij = sredisce(ladje)
        optimalna_razporeditev.append(sredisce_ladij)
    return(optimalna_razporeditev)
...
def optimiraj_ladje(marsovci: Flota, ladje: Flota) -> Flota:
    ...
    """
    marsovci2 = skupine_marsovcev(marsovci, ladje)
    nove_ladje = razpostavi_ladje(marsovci2)
    stare_ladje = []

    while nove_ladje != stare_ladje:
        stare_ladje = nove_ladje
        marsovci2 = skupine_marsovcev(marsovci, stare_ladje)
        nove_ladje = razpostavi_ladje(marsovci2)
